    ...
    registerConnection: function (connection, collections, cb) {
      ...
      database.createPool(connections[connection.identity]).then(function () {
        if(process.env.NODE_ENV == 'development'){
          Object.keys(_collections).forEach(function (collectionName) {
            if (_collections[collectionName].migrate == "alter"){
              //Lets first check if the table already exists
              var definition = _collections[collectionName].definition;
              run(database, buildQuery(OPERATIONS.DEFINE, { name: collectionName, def: definition }, null, null), null, function (error, result) {
                var _isTableAlreadyCreated = function (error) {
                  const ORACLE_TABLE_ALREADY_EXISTS_CODE = "00955";
                  return error && error.message && error.message.indexOf(ORACLE_TABLE_ALREADY_EXISTS_CODE) >= 0;
                };
                if (_isTableAlreadyCreated(error)) {
                  //If table exists, lets check for model alterations to reflect them on database
                  var executions = 0;
                  var totalColumns = Object.keys(definition).length;
                  Object.keys(definition).forEach(function(d){
                    var def = {};
                    def[d] = definition[d];
                    run(database, buildQuery(OPERATIONS.ALTER, { name: collectionName, def: def }, null, null), null, function(err, rsp){
                      executions++;
                      if(executions == totalColumns) cb();
                    });
                  })
                } else {
                  //If table does not exist, simply forward callback
                  cb();
                }
              });
            }
